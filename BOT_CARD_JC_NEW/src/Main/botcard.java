...
public class botcard extends Applet implements ExtendedLength
{
	...
	public final static short MAX_BALANCE = 1000;
	// Cau hinh PIN
	...
	// INS - GET PIN FOR VERIFY CARD
	private final static byte INS_PIN_CHECK = 0x1A;
	private final static byte INS_VERIFY_CARD = 0x1B;
	
	// INS / P1 - TRANSACTION
	private final static byte INS_TRANSACTION = 0x1C;
	private final static byte P1_THANH_TOAN = 0x01;
	private final static byte P1_NAP_TIEN = 0x02;
	private final static byte P1_SO_DU = 0x03;
	...
	public void process(APDU apdu)
	{
		...
		case INS_TRANSACTION:
			short p1_trans = buf[ISO7816.OFFSET_P1];
			switch(p1_trans) {
				case P1_THANH_TOAN:
					thanhToan(apdu);
					break;
...
private void sendPublicKey(APDU apdu) {
	...
	private void checkPin(APDU apdu) {
		...
		boolean isCheck = pin.check(buffer,ISO7816.OFFSET_CDATA, pinLength);
		
		// If isCheck = false -> Exception
		if(!isCheck) {
	...
	private void signData(APDU apdu) {
		...
		byte[] dataSign = JCSystem.makeTransientByteArray(dataLength, JCSystem.CLEAR_ON_RESET);
		Util.arrayCopy(buffer,ISO7816.OFFSET_CDATA,dataSign,(short)0,dataLength);
		
		rsaSignature.init(rsaPrivateKey, Signature.MODE_SIGN);
		short signLength = rsaSignature.sign(dataSign, (short)0, dataLength, buffer, (short)0);
		
		apdu.setOutgoing();
		apdu.setOutgoingLength(signLength);
		apdu.sendBytes((short)0,signLength);
	}
	...
	private void thanhToan(APDU apdu) {
		...
		short amount = (short) ((buffer[ISO7816.OFFSET_CDATA] << 8) | (buffer[ISO7816.OFFSET_CDATA+1] & 0xFF));
	...
	private void hienThiSoDu(APDU apdu) {
		byte[] buffer = apdu.getBuffer();
		
		// convert short -> byte
		buffer[0] = (byte) (balance >> 8); // Byte cao
		buffer[1] = (byte) (balance & 0xFF); // Byte thp
		
		
		apdu.setOutgoingAndSend((short) 0, (short) 2); // Gi 2 byte s d
	}
}